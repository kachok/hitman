...
def bufferupdatedb(assignment, worker, correct, total):
	...
	if(currapp < FREEBIES):
		...
		print currstatusdesc, currstatus
		if(currstatusdesc == "PENDING" or currstatusdesc == "PREAPPROVAL"):
			...
			if(currstatusdesc =="APPROVED"):
				...
				print "ELSE", currstatusdesc, currstatus
				sql="INSERT INTO esl_grades (assignment_id, worker_id, status) VALUES (%s,%s,%s);" 	
				cur.execute(sql, (assignment, worker, "REJECT"))
				ret = 0	
	conn.commit()
...
def printfeedbacks(assignment, worker):
        ofile = open('feedbacks.log', 'a')
        ofile.write(str(assignment)+'\t'+str(worker)+'\n')     
        ofile.write(get_feedback_buf(worker, assignment)+'\n\n')
        ofile.close()  

def get_feedback_buf(worker, assignment):
	conn = psycopg2.connect("dbname='"+settings["esl_dbname"]+"' user='"+settings["user"]+"' host='"+settings["host"]+"'")
	cur = conn.cursor()
	sql = "SELECT * from esl_appr_buffer WHERE worker_id=%s;"
	...
	if(status == "REJECT" and desc == "PENDING"):
		return reject_pending_feedback_buf(assignment, worker)
	if(status == "APPROVE" and desc == "PENDING"):
		return accept_feedback()
	if(status == "APPROVE" and desc == "PREAPPROVAL"):
		return preapprove_feedback_buf(assignment, worker)
	if(status == "REJECT" and desc == "BLOCKED"):
		return reject_blocked_feedback_buf(assignment, worker)
...
def reject_pending_feedback_buf(assignment, worker):
        conn = psycopg2.connect("dbname='"+settings["esl_dbname"]+"' user='"+settings["user"]+"' host='"+settings["host"]+"'")
        cur = conn.cursor()
        ...
        sql = "SELECT average from esl_appr_buffer where worker_id=%s;"
        cur.execute(sql, (str(worker), ))
        avg = int(100*cur.fetchone()[0])
        reject_feedback="We are sorry, but your responses did not meet our quality control requirement. Our control sentence was: %s Your correction was: %s We were looking for: %s Currently, your average accuracy on our controls is %s%%. If it falls below 20%% we will stop accepting your hits. Next time, be sure to read all the sentences carefully before submitting. We appreciate your effort." % (our_sent,their_sent,orig_sent,str(avg))
...
def preapprove_feedback(assignment, worker):
	...
	sql = "SELECT average from esl_workers where worker_id=%s;"
        cur.execute(sql, (str(worker), ))
	avg = int(100*cur.fetchone()[0])
	reject_feedback="Thank you! Currently, over the past HITs you have submitted to us, your average accuracy on our controls has been %s%%. We will accept your first 10 HITs for free; after that, we cannot accept HITs from workers with averages below 20%%." % (str(avg))
	return reject_feedback

def preapprove_feedback_buf(assignment, worker):
        conn = psycopg2.connect("dbname='"+settings["esl_dbname"]+"' user='"+settings["user"]+"' host='"+settings["host"]+"'")
        cur = conn.cursor()
        sql = "SELECT average from esl_appr_buffer where worker_id=%s;"
        cur.execute(sql, (str(worker), ))
...
def reject_blocked_feedback(assignment, worker):
	...
	sql = "SELECT average from esl_workers where worker_id=%s;"
        cur.execute(sql, (str(worker), ))
	avg = int(100*cur.fetchone()[0])
	reject_feedback="We are sorry, but your responses did not meet our quality control requirement. Over the past HITs you have submitted to us, your average accuracy on our controls has been %s%%. We cannot accepted averages below 20%%." % (str(avg))
	return reject_feedback


def reject_blocked_feedback_buf(assignment, worker):
        conn = psycopg2.connect("dbname='"+settings["esl_dbname"]+"' user='"+settings["user"]+"' host='"+settings["host"]+"'")
        cur = conn.cursor()
        sql = "SELECT average from esl_appr_buffer where worker_id=%s;"
        cur.execute(sql, (str(worker), ))
