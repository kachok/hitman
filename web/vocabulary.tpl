...
<body>
 ...
 $.getJSON("/words?hitId={{params['hitid']}}", function(data) {
	for (word in data["words"]){
		...
		html='<tr '+c+' ><td id="word'+word+'"><!--'+data["words"][word]["word"]+'--><img src="/static/images/'+data["words"][word]["word_id"]+'.png"/></td><td><input id="word'+word+'in" type="text" name="word'+word+'" size="50"></input><br/><div id="word'+word+'help" style="display:none"><img src="/static/images/'+data["words"][word]["word_id"]+'_sentences.png"/></div></td><td width="400px"><label for="check'+word+'">Can\'t translate</label> <input type="checkbox" name="check'+word+'" id="check'+word+'"></input> <div id="reason'+word+'div" style="display:none;"> '+options+'</div></td></tr>';
		//alert(html);
